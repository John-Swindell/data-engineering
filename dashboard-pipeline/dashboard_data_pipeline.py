"""
Dashboard Data Acquisition Pipeline

This script creates a comprehensive dataset for the user-facing SaaS dashboard.
It fetches the CURRENT Top 200 cryptocurrencies by market cap and then acquires
their full historical market, on-chain, and social data from CoinGecko,
DeFiLlama, and LunarCrush.

It leverages the same shared GCS caching system as the research pipeline to
ensure fast and efficient execution.
...
#         bucket_name='cryptonest-pipeline-cache'  # TODO  <--- IMPORTANT: REPLACE WITH THE BUCKET NAME FROM ARSLAN
#     )
...
if __name__ == '__main__':
    ...
    # --- 5. Define Product Universe & Build Lookup Maps ---
    print("\n5. Defining Product Universe & Building Lookup Maps...")
    try:
        # --- Define Universe from Live Market Cap ---
        print("   -> Fetching Top 200 Product Universe from CoinGecko...")
        ...
        product_universe_coin_ids = [coin['id'] for coin in top_200_markets]
        ticker_map = {coin['id']: coin['symbol'].upper() for coin in top_200_markets}
        print(f"   -> Successfully defined universe with {len(product_universe_coin_ids)} assets.")

    except Exception as e:
        raise SystemExit(f"FATAL: Could not fetch Top 200 universe from CoinGecko. Error: {e}")

    # Bundle headers for easy passing
    all_headers = {'llama': llama_headers, 'lunarcrush': lunarcrush_headers}

    # Use the cacher to get the maps
    ...
    # --- 6. Fetch Full History for the Product Universe ---
    print(f"\n--- Fetching full history for {len(product_universe_coin_ids)} product assets ---")
    all_coin_data = []
    for coin_id in product_universe_coin_ids:
        ...
        # --- 7. Final Combination & Save ---
    # No need for point-in-time assembly here
    print("\n7. Combining, cleaning, and saving final dataset...")
    if all_coin_data:
        master_df = pd.concat(all_coin_data, ignore_index=True)

        # Final cleaning and deduplication logic
        ...
        print("\n--- Inspecting DataFrame before saving ---")
        master_df.info(verbose=True)

        master_df.to_parquet(OUTPUT_FILE)
        print(f"\n✅ PROCESS COMPLETE! Dashboard data saved to '{OUTPUT_FILE}'.")
